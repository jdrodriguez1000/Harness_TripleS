...
- **Actualización (T-021, 2026-07-23):** Refinamiento del mecanismo para `ClaudeSDKProvider`, sin invalidar la decisión. El Claude Agent SDK arma el entorno del subproceso como `{**os.environ, **options.env}`: no permite *borrar* una variable heredada vía `options.env`, solo sobrescribirla. Se sobrescriben `ANTHROPIC_API_KEY`/`ANTHROPIC_AUTH_TOKEN` a cadena vacía, que el CLI trata como ausente. Ver L-018.
...
### D-038 — La sesión persistente se encapsula tras una nueva abstracción `Sesion`, no extendiendo `Provider`

- **Fecha:** 2026-07-23
- **Contexto:** Al implementar T-021, D-036 ya anticipaba que `Provider.send(prompt) -> str` (D-008, de un solo disparo) "evoluciona" hacia algo multi-turno, pero no fijaba la forma concreta: extender `Provider` con un método nuevo, mutar su contrato, o crear una abstracción hermana.
- **Decisión:** La persistencia de contexto se modela con una abstracción nueva y separada, `Sesion` (ABC async en `src/soda/core/sesion.py`, gestor de contexto `__aenter__`/`__aexit__` + `async def enviar(prompt) -> str`), simétrica a `Provider` pero no derivada de ella. `Provider.send()` de un disparo queda intacto.
- **Alternativas descartadas:** Añadir un método multi-turno a la propia clase `Provider` (p. ej. `Provider.abrir_sesion()` como método abstracto más), descartado porque forzaría a todo `Provider` existente (incluido `ClaudeCLIProvider`, usado por agentes de un solo turno como `SesionStarter`) a implementar un contrato que no necesita; mutar `send()` para que opcionalmente mantenga estado, descartado porque mezclaría dos contratos de vida útil distinta (un disparo vs. una conversación) en una sola interfaz.
- **Consecuencias:** Los agentes de un solo turno (`SesionStarter`) no se tocan ni se ven afectados por la nueva abstracción. El backend concreto (hoy el Claude Agent SDK) queda contenido dentro de `SesionClaudeSDK`/`ClaudeSDKProvider` (`src/soda/providers/claude_sdk.py`) y nunca cruza la frontera de `Sesion` como tipo del SDK, protegiendo D-006/D-008. Futuro orquestador REPL (T-022) programa contra `Sesion`, no contra `ClaudeSDKClient`.

### D-039 — `claude-agent-sdk` pasa de extra `[spike]` a dependencia dura del producto

- **Fecha:** 2026-07-23
- **Contexto:** Desde T-020, `claude-agent-sdk` vivía en el extra opcional `[project.optional-dependencies] spike`, reservado a los scripts de exploración fuera de producto (C-003), a la espera de que el spike confirmara viabilidad antes de comprometerlo. T-021 construye sobre el SDK dentro de `src/soda/` (código de producto, no un script), lo que ya no encaja con un extra opcional.
